...
const LINEITEMS_SEARCH_QUERY = gql`
    query LINEITEMS_SEARCH_QUERY($search:String!){
        lineItems(where:{name_contains:$search}){
            id
            name
        }
    }
    ...
    searchHandler = async(client,e)=>{
        ...
        try{
            let res = await client.query({
                query:LINEITEMS_SEARCH_QUERY,
                variables:{
                    search:e.target.value
                }
            });
            this.setState({
                results:[...res.data.lineItems],
                searchLoading:false,
            });
        }catch(err){       
            this.setState({
